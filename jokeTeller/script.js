...
function test () {
    VoiceRSS.speech({
        key: '02cf6e13d2ed40beb125c1ba1f548b06',
        src: jokeData.setup + jokeData.delivery ,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
        c: 'mp3',
...
async function getJokes(){
    const apiUrl = 'https://v2.jokeapi.dev/joke/Programming?blacklistFlags=nsfw,religious,political,racist&type=twopart'
    try{
        const jokeJson = await fetch(apiUrl);
        jokeData = await jokeJson.json();
        toggleButton();
        test();
    }catch(err){
